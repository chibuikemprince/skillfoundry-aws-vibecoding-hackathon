...
console.log({id: savedCurriculum._id})
...
// console.log({resources})
  ...
  async generateLessonForSubtopic(subtopicId: string, skill: string, level: string, curriculumId: string): Promise<any> {
    // Check if lesson already exists
    let lesson = await LessonModel.find({ subtopicId, curriculumId });
    
    if (lesson.length == 0) {
      // Generate new lesson
      const lessonData = await llmService.generateLesson(subtopicId, skill, level);

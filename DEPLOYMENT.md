...
# Deploy and start
pm2 start dist/server.js --name "skillfoundry-backend"
pm2 startup
pm2 save
```
...
heroku create skillfoundry-backend
...
```env
PORT=3000
MONGODB_URI=mongodb+srv://user:pass@cluster.mongodb.net/skillfoundry
JWT_SECRET=super-secure-random-string-change-this
NODE_ENV=production
FRONTEND_URL=https://skillfoundry-frontend.netlify.app
...
### Frontend (.env)
```env
REACT_APP_API_URL=https://skillfoundry-backend.herokuapp.com/api
```
...
app.use(cors({
  origin: [
    'http://localhost:3001',
    'https://skillfoundry.netlify.app',
